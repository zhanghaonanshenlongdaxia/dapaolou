...
foreach (var guid in rendererGuids)
{
    ...
    if (ssaoType == null) { Debug.Log("SSAO type missing: " + path); continue; }
    ...
    if (map != null) { map.arraySize++; map.GetArrayElementAtIndex(map.arraySize - 1).objectReferenceValue = ssao; }
...
Debug.Log("SSAO injected: " + ssaoAdded);
...
foreach (var light in Object.FindObjectsOfType<Light>())
{
    if (light.type == LightType.Directional)
    {
        ...
        Debug.Log("Sun shadow strength=0.9: " + light.name);
    }
}
